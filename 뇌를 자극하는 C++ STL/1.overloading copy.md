...
- **함수 호출** 연산자 `operator()`
  ...
  - `Func()`가 함수 포인터 :point_right: 함**_수 포인터_** 
  - `Func()`가 함수 객체 :point_right: **_함수 객체_**
...
```note
- 명시적(explicit) 호출 :call_me_hand:
- 암시적(implicit) 호출 :call_me_hand:
...
- 타입 변환 연산자 오버로딩을 이용한 타입 변환 :hand_over_mouth:
	- 자신의 타입을 다른 타입으로 변환
